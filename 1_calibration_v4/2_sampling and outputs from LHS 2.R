...
parameter_ranges_klebsiella1 <- list(
  beta_h = c("qunif", list(min = 0.1084, max = 0.1257)), #recruitment rate and natural mortality rate
  kappa_h = c("qunif", list(min = 0.0005, max = 0.0034)), #the daily rate of progression from colonisation to BSI in the hospital
  kappa_1 = c("qunif", list(min = 0.0000184, max = 0.0000456)) #the daily rate of progression from colonised state to BSI in the community
)
#==============================================================================================
...
write.csv(lhs_samples_klebsiella1_df, "1_calibration_v4/sampling_outputs/samples2.csv", row.names = FALSE)


#---------------------------------------------------#
# output from the second batch of combinations ####
#---------------------------------------------------#
...
parms_comb1 <- read.csv("1_calibration_v4/sampling_outputs/samples2.csv")
...
for(i in 1:nrow(grid)){
  ...
  df.calibration_2<-bind_rows(df.calibration_2, tmp)
}

View(df.calibration_2)

# save to csv
##write.csv(df.calibration_2, "1_calibration_v4/calibration_outputs/csv/klebsiella_1_calibration_2.csv", row.names = FALSE)

# check output # plot the outputs of hospital population at equilibrium
cases_calibration_2 <- df.calibration_2 %>% 
  mutate(year = floor((as.integer(time) - 1) / 365) + 1)

View(cases_calibration_2)

nhs_trust <- 134

cases_grouped_by_year_calibration_2 <- cases_calibration_2 %>%
...
View(cases_grouped_by_year_calibration_2)

# save to csv
#write.csv(cases_grouped_by_year_calibration_2, "1_calibration_v4/calibration_outputs/csv/klebsiella_1_calibration_2_bsi_incidence_summarised_per_year.csv", row.names = FALSE)
...
check_equilibrium_calibration_2 <- cases_grouped_by_year_calibration_2 %>% 
...
View(check_equilibrium_calibration_2) #ok


# save to csv
write.csv(check_equilibrium_calibration_2, "1_calibration_v4/calibration_outputs/csv/klebsiella_1_calibration_2_eq_values.csv", row.names = FALSE)

# turn off scientific notation
options(scipen = 999)

# load the output
calibration_output_1 <- read.csv("1_calibration_v4/calibration_outputs/csv/klebsiella_1_calibration_2_eq_values.csv")
...
df.long <- df %>% filter(total_incidence_all_trust > 4000, 
                         prop_bsi_hospitalised_all_trust > 50,
                         colonisation_prevalence_comm_percent_all_trust >1.5, colonisation_prevalence_comm_percent_all_trust <3.5)
...
# combinations from set 1 
# beta_h : 0.1084 to 0.1257
# kappa_h: 0.0005 to 0.0034
# kappa_1: 0.0000184 to 0.0000456

# combinations from set 2 
# beta_h : 0.1142 to 0.1204
# kappa_h: 0.00104 to 0.00148
# kappa_1: 0.0000431 to 0.0000451
...
# save plot
ggsave("1_calibration_v4/calibration_outputs/plots/klebsiella_1_calibration_2_outputs_plot.png", width = 18.5, height = 10, dpi = 300)
