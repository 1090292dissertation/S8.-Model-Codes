#### K pneumoniae transmission model - entire older adults population - basic model - turn off intervention ####

#### Working directory is not set up since this is an R project. OPEN the R project file "kleb_codes_4" to run all of the codes. ####
...
install_if_not_installed(c("deSolve", "ggplot2", "dplyr", "reshape2", "tidyverse", 
                           ...
                           "writexl", "git2r", "ggtext"))

# load multiple libraries
lapply(c("deSolve", "ggplot2", "dplyr", "reshape2", "tidyverse", "readr", "readxl", 
         "lubridate", "purrr", "data.table", "writexl", "git2r", "scales", "ggtext"), library, character.only = TRUE)
...
klebsiella_1 <- function(time, state, parameters){
  with(as.list(c(state, parameters)), {
    ...
    P_hosp_c1 <- S_h1 + C_h1 + I_h1 + VS_h1 + VC_h1  #define total 65+ year-olds population 
    
    # total population outside the hospital (within the community)
    P_community <- S_c1 + C_c1 + I_c1 + VS_c1 + VC_c1 #define total 65+ year-olds population in the community
    
    # total older adults population
    P1 <- P_hosp_c1 + P_community #define total 65+ year-olds population
    
    # infectious populations   
    I_hosp_c1 <- C_h1 + I_h1 + VC_h1 #define size of infectious population for the entire older adults population in the hospital
    ...
    #-----------------------------------#
    #maintaining fixed hospital capacity#
    #-----------------------------------#
    ...
    non_bsi_entry_pool <- S_c1 + C_c1 + VS_c1 + VC_c1 #non-bsi entry pool in the community
    ...
    topup_Sc1 <- S_c1/non_bsi_entry_pool*avail_beds #proportion of unvaccinated susceptible in the community
    
    topup_Cc1 <- C_c1/non_bsi_entry_pool*avail_beds #proportion of unvaccinated colonised in the community
    
    topup_VSc1 <- VS_c1/non_bsi_entry_pool*avail_beds #proportion of vaccinated susceptible in the community
    
    topup_VCc1 <- VC_c1/non_bsi_entry_pool*avail_beds #proportion of vaccinated colonised in the community
    ...
    dI_c1 <- kappa_1*C_c1 - sigma_1b*I_c1 + - mu_1*I_c1 + (1-rho_1)*kappa_1*VC_c1
    
    #community vaccinated
    dVS_c1 <- gamma_1*VS_h1 + epsilon_1*VC_c1 - sigma_1a*VS_c1 - topup_VSc1 - mu_1*VS_c1 
    dVC_c1 <- gamma_1*VC_h1 - epsilon_1*VC_c1 - sigma_1a*VC_c1 - topup_VCc1 - mu_1*VC_c1 - (1-rho_1)*kappa_1*VC_c1
    ...
    dI_h1 <- kappa_h*C_h1 + sigma_1b*I_c1 - tau_h*I_h1 - omega_h*I_h1 - mu_1*I_h1 + (1-rho_1)*kappa_h*VC_h1
    
    #hospital vaccinated
    dVS_h1 <- sigma_1a*VS_c1 + topup_VSc1 - lambda_h*VS_h1 - gamma_1*VS_h1 - mu_1*VS_h1  
    dVC_h1 <- sigma_1a*VC_c1 + topup_VCc1 + lambda_h*VS_h1 - gamma_1*VC_h1 - mu_1*VC_h1 - (1-rho_1)*kappa_h*VC_h1
    ...
    dD_h1 <- (mu_1 + omega_h)*I_h1 
    
    # incidence counter for community and hospital
    dCInc_1a <- lambda_h*(S_h1 + VS_h1) # incidence of colonisation in the hospital
    dCInc_1b <- kappa_1*C_c1 + (1-rho_1)*kappa_1*VC_c1  # progression from colonised state to BSI state in the community 
    dCInc_1c <- sigma_1b*I_c1 # BSI hospitalisations or admitted as BSIs
    dCInc_1d <- kappa_h*C_h1 + (1-rho_1)*kappa_h*VC_h1 # hospital-onset cases 
    
    # Colonised upon admissions
    dcol_adm <- sigma_1a*(C_c1 + VC_c1)
    
    # Colonised upon discharges
    dcol_discharges <- gamma_1*(C_h1 + VC_h1)
    
    # Total admissions and discharges
    d_admissions <- sigma_1a*(S_c1 + VS_c1 + C_c1 + VC_c1) + sigma_1b*I_c1 + topup_Sc1 + topup_VSc1 + topup_Cc1 + topup_VCc1
    d_discharges <- gamma_1*(S_h1 + VS_h1 + C_h1 + VC_h1)
    
    # list down the ode #ensure the order is the same as the order of the equations above!
    list(c(S_c1 = dS_c1, C_c1 = dC_c1, I_c1 = dI_c1, VS_c1 = dVS_c1, VC_c1 = dVC_c1, 
           S_h1 = dS_h1, C_h1 = dC_h1, I_h1 = dI_h1, VS_h1 = dVS_h1, VC_h1 = dVC_h1,
           D_h1 = dD_h1, CInc_1a = dCInc_1a, CInc_1b = dCInc_1b, CInc_1c = dCInc_1c, CInc_1d = dCInc_1d,
           col_adm = dcol_adm, col_discharges = dcol_discharges,
           admissions = d_admissions, discharges = d_discharges)) 
  })
}

# implement vaccination with event function outside the ODE
# this is to ensure that the vaccination is implemented at one time point
vaccination_event <- function(time, state, parameters) {
  
  with(as.list(state), {
    
    message("Event at time: ", time) # print the timestamp of the event
    
    # calculate the number of susceptibles and colonised individuals to be vaccinated
    
    vacc_coverage <- parameters1["vacc_coverage"] # vaccination coverage 
    
    num_vaccinated_Sc1 <- vacc_coverage * S_c1 
    num_vaccinated_Cc1 <- vacc_coverage * C_c1
    
    # update the state variables
    # vaccinate susceptibles
    S_c1 <- S_c1 - num_vaccinated_Sc1
    VS_c1 <- VS_c1 + num_vaccinated_Sc1
    
    # vaccinate colonised
    C_c1 <- C_c1 - num_vaccinated_Cc1
    VC_c1 <- VC_c1 + num_vaccinated_Cc1
    
    
    return(c(S_c1 = S_c1, C_c1 = C_c1, I_c1 = I_c1, VS_c1 = VS_c1, VC_c1 = VC_c1, 
             S_h1 = S_h1, C_h1 = C_h1, I_h1 = I_h1, VS_h1 = VS_h1, VC_h1 = VC_h1,
             D_h1 = D_h1, CInc_1a = CInc_1a, CInc_1b = CInc_1b, CInc_1c = CInc_1c, CInc_1d = CInc_1d,
             col_adm = col_adm, col_discharges = col_discharges,
...
source("4_sensitivity_analysis_v4/sensitivity_inputs/inputs_v4_sensitivity_analysis_model1.R")$value
#==================================================================================================

#==================================================================================================
#### 2. Setup initial values #### #ensure the order is the same as the order of the equations above
#==================================================================================================
istate1 <- c(S_c1 = S_c1, C_c1 = C_c1, I_c1 = I_c1, VS_c1 = VS_c1, VC_c1 = VC_c1, 
             S_h1 = S_h1, C_h1 = C_h1, I_h1 = I_h1, VS_h1 = VS_h1, VC_h1 = VC_h1,
             D_h1 = D_h1, CInc_1a = CInc_1a, CInc_1b = CInc_1b, CInc_1c = CInc_1c, CInc_1d = CInc_1d,
...
parameters1 <- c(mu_1 = mu_1, #recruitment rate and natural mortality rate
                 ...
                 kappa_1 =  kappa_1, #the daily rate of progression from colonised state to BSI in the community
                 kappa_h =  kappa_h, #the daily rate of progression from colonisation to BSI in the hospital
                 ...
                 vacc_coverage = 0, #default coverage is 0
                 rho_1 = 0) #default vaccine efficacy is 0
...
#event_list <- list(func = vaccination_event,
                   #time = c(365*0)) #time at which the vaccination event occurs -> the solver will implement it at t+1 or time 1

#===============================================================
#### 4. Setup timesteps ####
#===============================================================
start1 <- 0 #start time for the simulation
end1 <- 365*5 #end point of the timestep #we have gotten rid of the run-in (settling-in) period by using the equilibrium values from calibration outputs as initial values
...
#if (tail(out1[, "S_c1"], 1) - tail(out1[, "S_c1"], 2)[1] < 1e-6 & 
#tail(out1[, "C_c1"], 1) - tail(out1[, "C_c1"], 2)[1] < 1e-6 & 
#tail(out1[, "I_c1"], 1) - tail(out1[, "I_c1"], 2)[1] < 1e-6) {
...
#write.csv(out1, file = "4_sensitivity_analysis_v4/sensitivity_outputs/check_manually_.csv") #save the output to a csv file