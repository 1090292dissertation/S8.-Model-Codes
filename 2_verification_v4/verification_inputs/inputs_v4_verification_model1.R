# sub-script to store parameters values and initial values for the model verification

# load necessary library
library(data.table)

# load initial values and parameters from CSV files
initial_values <- read.csv("2_verification_v4/verification_inputs/inputs - initial_values - for verification - model1.csv", header = TRUE, stringsAsFactors = FALSE) 
parameters <- read.csv("2_verification_v4/verification_inputs/inputs - parameters_v5 - for verification - model1.csv", header = TRUE, stringsAsFactors = FALSE) 
