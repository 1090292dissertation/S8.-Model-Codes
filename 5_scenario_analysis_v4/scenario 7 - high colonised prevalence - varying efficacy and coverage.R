...
#-------------------------------------------------------------------------------------------------#
# Generate outcomes for low percentage of hospital population out of total population scenario #### 
#-------------------------------------------------------------------------------------------------#
...
istate1["S_c1"] <-  80470 - S_c1 * ph - C_c1 * ph #keep the total population the same while modifying proportion of patients in the hospital to allow for higher prevalence of colonised in the community for scenario analysis 
istate1["C_c1"] <-  80470 - C_c1 * ph #keep the total population the same while modifying modify proportion of patients in the hospital to allow for higher prevalence of colonised in the community for scenario analysis 
istate1["C_c1"] <-  0; #keep the total population the same while modifying modify proportion of patients in the hospital to allow for higher prevalence of colonised in the community for scenario analysis 
istate1["S_h1"] <-  S_c1 * ph #modify proportion of patients in the hospital to allow for higher prevalence of colonised in the community for scenario analysis 
istate1["C_h1"] <-  C_c1 * ph #modify proportion of patients in the hospital to allow for higher prevalence of colonised in the community for scenario analysis 
istate1["I_h1"] <-  0 #modify proportion of patients in the hospital to allow for higher prevalence of colonised in the community for scenario analysis 
...
event_list <- list(func = vaccination_event,
                   time = c(34674)) #time at which the vaccination event occurs -> the solver will implement it at t+1 
...
# save as csv
write.csv(eq_values, file = "5_scenario_analysis_v4/scenario_outputs/csv/eq_values_low_prevalence_smaller_hosp_size.csv")

#====================================================================================
# discard non-equilibrium values to implement vaccination with shorter running time
#====================================================================================
# initial values at equilibrium
eq_values <- read.csv("5_scenario_analysis_v4/scenario_outputs/csv/eq_values_low_prevalence_smaller_hosp_size.csv")

# use equilibrium values to shorten running time for intervention
istate1["S_h1"] <- eq_values$S_h1
istate1["C_h1"] <- eq_values$C_h1 #unchanged
...
df.scen7<-NULL # warning! the data frame stores the results so the next outputs will be amended based on the last value stored
for(i in 1:nrow(grid)){
  ...
  tmp <- as_tibble(as.data.frame(run)) %>%  
    ...
    select(everything())
  df.scen7<-bind_rows(df.scen7, tmp) 
}
View(df.scen7)

View(df.scen7 %>% filter(time > 1823))

# add the number of vaccine column 
# get the number of vaccines
num_vaccines <- df.scen7 %>% filter(time == 1)

View(num_vaccines)

df.scen7_long <- df.scen7 %>% 
  pivot_longer(cols = -c(time, vacc_coverage, vacc_efficacy, vacc_cov_percent, vacc_efficacy_percent, community_prevalence),
               names_to = "variable", 
               values_to = "value")
# save as RDS
#saveRDS(df.scen7_join, file = "3_intervention_v4/intervention_outputs/df.scen7.rds")

#saveRDS(df.scen7_long, file = "3_intervention_v4/intervention_outputs/df.scen7_long.rds")
...
total_bsi_cases_with_scen7_hosp <- df.scen7_long %>% 
  filter(variable == "incidence_bsi_hosp") %>% #Filter solutions 
  filter(time>start_time, time<=end_time) %>% 
  mutate(year = floor((as.integer(time) - 1) / 365) + 1)

total_bsi_cases_with_scen7_grouped <- total_bsi_cases_with_scen7_hosp %>% 
...
total_bsi_cases_with_scen7_grouped

View(total_bsi_cases_with_scen7_grouped)

# total number of cases over 5 years 
total_number_of_cases_5_years <- total_bsi_cases_with_scen7_grouped %>% 
...
write.csv(total_bsi_cases_with_scen7_grouped, "5_scenario_analysis_v4/scenario_outputs/csv/cases_per_year_with_scen7.csv")

write.csv(total_number_of_cases_5_years, "5_scenario_analysis_v4/scenario_outputs/csv/cases_5_years_with_scen7.csv")
...
# all years
cum_cases_5_years <- read.csv("5_scenario_analysis_v4/scenario_outputs/csv/cases_per_year_with_scen7.csv")
...
# save to csv for use later
write.csv(cum_cases_averted, "5_scenario_analysis_v4/scenario_outputs/csv/_5_years_with_scen7.csv")


## calculate the total number of cases averted
cases_averted_scenario7 <- cum_cases_averted %>% 
  mutate(cases_averted_cum = abs(with_int_baseline_cases_cum - no_int_cases_cum))

View(cases_averted_scenario7)


# select the total number of deaths averted over 5 years
total_cases_averted_scenario7 <- cases_averted_scenario7 %>% filter(year == 5, vacc_coverage != 0, vacc_efficacy !=0)

total_cases_averted_scenario7$percent_reduction_cases <- 
  round((total_cases_averted_scenario7$cases_averted_cum/total_cases_averted_scenario7$no_int_cases_cum),3)*100

View(total_cases_averted_scenario7)

total_cases_averted_scenario7 <- total_cases_averted_scenario7 %>% mutate(vacc_cov_percent = vacc_coverage*100,
...
total_cases_averted_scenario7
View(total_cases_averted_scenario7)

total_cases_averted_scenario7_join <- total_cases_averted_scenario7 %>% 
  left_join(lookup_num_vaccines, by = "vacc_cov_percent")

View(total_cases_averted_scenario7_join)

total_cases_averted_scenario7_join$cases_averted_per_vaccine <- 
  round((total_cases_averted_scenario7_join$cases_averted_cum/total_cases_averted_scenario7_join$total_num_vaccination),4)

total_cases_averted_scenario7_join$cases_averted_per_1000_vaccine <- 
  total_cases_averted_scenario7_join$cases_averted_per_vaccine*1000

total_cases_averted_scenario7_join$num_vacc_per_one_case_averted <- 
  round((total_cases_averted_scenario7_join$total_num_vaccination/total_cases_averted_scenario7_join$cases_averted_cum),0)

View(total_cases_averted_scenario7_join)

total_cases_averted_scenario7_join2 <- total_cases_averted_scenario7_join %>% filter(vacc_cov_percent %in% c(10, 40, 70, 100), vacc_eff_percent %in% c(10, 40, 70, 100))

View(total_cases_averted_scenario7_join2)

percent_reduction_quantiles <- quantile(total_cases_averted_scenario7_join$percent_reduction_cases, probs = c(0, 0.25, 0.75, 1))

plot_cases_scenario7_percent_reduction <- ggplot(total_cases_averted_scenario7_join,
  ...
  geom_text(data = filter(total_cases_averted_scenario7_join, 
  ...
  scale_x_continuous(breaks = sort(unique(total_cases_averted_scenario7_join$vacc_cov_percent)),
                     labels = paste0(sort(unique(total_cases_averted_scenario7_join$vacc_cov_percent)))) +
  
  scale_y_continuous(breaks = sort(unique(total_cases_averted_scenario7_join$vacc_eff_percent)),
                     labels = paste0(sort(unique(total_cases_averted_scenario7_join$vacc_eff_percent)))) +
  labs(
    title = "Scenario 7 (large hospital population)\nPercentage reduction in the total number of cases\n(over 5 years)",
    x = "Vaccine coverage (%)",
    y = "Vaccine efficacy (%)",
    fill = NULL
  ) + 
...
ggsave(plot_cases_scenario7_percent_reduction, file = "5_scenario_analysis_v4/scenario_outputs/plots/scen7/plot_cases_scenario7c_percent_reduction.png", width = 7, height = 7)



plot_cases_scenario7_efficiency1 <- ggplot(total_cases_averted_scenario7_join,
...
ggsave(plot_cases_scenario7_efficiency1, file = "5_scenario_analysis_v4/scenario_outputs/plots/scen7/plot_cases_scenario7c_efficiency1.png", width = 7, height = 7)


plot_cases_scenario7_efficiency2 <- ggplot(total_cases_averted_scenario7_join,
...
ggsave(plot_cases_scenario7_efficiency2, file = "5_scenario_analysis_v4/scenario_outputs/plots/scen7/plot_cases_scenario7c_efficiency2.png", width = 7, height = 7)



# save file for boxplot
boxplot.df.scen7 <- data.frame(
  percent_reduction = total_cases_averted_scenario7_join$percent_reduction_cases,
  scenario = "Scenario 7 (large hospital population size)"
)

# save as csv
write.csv(boxplot.df.scen7, "5_scenario_analysis_v4/scenario_outputs/csv/boxplotscen7.csv")

read.csv("5_scenario_analysis_v4/scenario_outputs/csv/boxplotscen7.csv")



# save file for boxplot
boxplot.df.scen7b <- data.frame(
  num_vacc_per_case = total_cases_averted_scenario7_join$num_vacc_per_one_case_averted,
  scenario = "Scenario 7 (large hospital population size)"
)

# save as csv
write.csv(boxplot.df.scen7b, "5_scenario_analysis_v4/scenario_outputs/csv/boxplotscen7b.csv")

read.csv("5_scenario_analysis_v4/scenario_outputs/csv/boxplotscen7b.csv")
