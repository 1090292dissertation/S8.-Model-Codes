...
# save as csv
write.csv(eq_values, file = "5_scenario_analysis_v4/scenario_outputs/csv/eq_values_high_transmissibility.csv")

#====================================================================================
# discard non-equilibrium values to implement vaccination with shorter running time
#====================================================================================
# initial values at equilibrium
eq_values <- read.csv("5_scenario_analysis_v4/scenario_outputs/csv/eq_values_high_transmissibility.csv")
...
df.scen5<-NULL # warning! the data frame stores the results so the next outputs will be amended based on the last value stored
for(i in 1:nrow(grid)){
  ...
  tmp <- as_tibble(as.data.frame(run)) %>%  
    ...
    select(everything())
  df.scen5<-bind_rows(df.scen5, tmp) 
}
View(df.scen5)

View(df.scen5 %>% filter(time > 1823))

# add the number of vaccine column 
# get the number of vaccines
num_vaccines <- df.scen5 %>% filter(time == 1)

View(num_vaccines)

df.scen5_long <- df.scen5 %>% 
  pivot_longer(cols = -c(time, vacc_coverage, vacc_efficacy, vacc_cov_percent, vacc_efficacy_percent, community_prevalence),
               names_to = "variable", 
               values_to = "value")
# save as RDS
#saveRDS(df.scen5_join, file = "3_intervention_v4/intervention_outputs/df.scen5.rds")

#saveRDS(df.scen5_long, file = "3_intervention_v4/intervention_outputs/df.scen5_long.rds")
...
total_bsi_cases_with_scen5_hosp <- df.scen5_long %>% 
  filter(variable == "incidence_bsi_hosp") %>% #Filter solutions 
  filter(time>start_time, time<=end_time) %>% 
  mutate(year = floor((as.integer(time) - 1) / 365) + 1)

total_bsi_cases_with_scen5_grouped <- total_bsi_cases_with_scen5_hosp %>% 
...
total_bsi_cases_with_scen5_grouped

View(total_bsi_cases_with_scen5_grouped)

# total number of cases over 5 years 
total_number_of_cases_5_years <- total_bsi_cases_with_scen5_grouped %>% 
...
write.csv(total_bsi_cases_with_scen5_grouped, "5_scenario_analysis_v4/scenario_outputs/csv/cases_per_year_with_scen5.csv")

write.csv(total_number_of_cases_5_years, "5_scenario_analysis_v4/scenario_outputs/csv/cases_5_years_with_scen5.csv")
...
# all years
cum_cases_5_years <- read.csv("5_scenario_analysis_v4/scenario_outputs/csv/cases_per_year_with_scen5.csv")
...
# save to csv for use later
write.csv(cum_cases_averted, "5_scenario_analysis_v4/scenario_outputs/csv/cases_5_years_with_scen5.csv")


## calculate the total number of cases averted
cases_averted_scenario5 <- cum_cases_averted %>% 
  mutate(cases_averted_cum = abs(with_int_baseline_cases_cum - no_int_cases_cum))

View(cases_averted_scenario5)


# select the total number of deaths averted over 5 years
total_cases_averted_scenario5 <- cases_averted_scenario5 %>% filter(year == 5, vacc_coverage != 0, vacc_efficacy !=0)

total_cases_averted_scenario5$percent_reduction_cases <- 
  round((total_cases_averted_scenario5$cases_averted_cum/total_cases_averted_scenario5$no_int_cases_cum),4)*100

View(total_cases_averted_scenario5)

total_cases_averted_scenario5 <- total_cases_averted_scenario5 %>% mutate(vacc_cov_percent = vacc_coverage*100,
...
total_cases_averted_scenario5
View(total_cases_averted_scenario5)

total_cases_averted_scenario5_join <- total_cases_averted_scenario5 %>% 
  left_join(lookup_num_vaccines, by = "vacc_cov_percent")

View(total_cases_averted_scenario5_join)

total_cases_averted_scenario5_join$cases_averted_per_vaccine <- 
  round((total_cases_averted_scenario5_join$cases_averted_cum/total_cases_averted_scenario5_join$total_num_vaccination),4)

total_cases_averted_scenario5_join$cases_averted_per_1000_vaccine <- 
  total_cases_averted_scenario5_join$cases_averted_per_vaccine*1000

total_cases_averted_scenario5_join$num_vacc_per_one_case_averted <- 
  round((total_cases_averted_scenario5_join$total_num_vaccination/total_cases_averted_scenario5_join$cases_averted_cum),0)

View(total_cases_averted_scenario5_join)

total_cases_averted_scenario5_join2 <- total_cases_averted_scenario5_join #%>% filter(vacc_cov_percent %in% c(10, 40, 70, 100), vacc_eff_percent %in% c(10, 40, 70, 100))

View(total_cases_averted_scenario5_join2)

percent_reduction_quantiles <- quantile(total_cases_averted_scenario5_join2$percent_reduction_cases, probs = c(0, 0.25, 0.75, 1))

plot_cases_scenario5_percent_reduction <- ggplot(total_cases_averted_scenario5_join2,
  ...
  geom_text(data = filter(total_cases_averted_scenario5_join2, 
  ...
  scale_x_continuous(breaks = sort(unique(total_cases_averted_scenario5_join2$vacc_cov_percent)),
                     labels = paste0(sort(unique(total_cases_averted_scenario5_join2$vacc_cov_percent)))) +
  
  scale_y_continuous(breaks = sort(unique(total_cases_averted_scenario5_join2$vacc_eff_percent)),
                     labels = paste0(sort(unique(total_cases_averted_scenario5_join2$vacc_eff_percent)))) +
  labs(
    title = "Scenario 5 (high transmissibility in hospital)\nPercentage reduction in the total number of cases\n(over 5 years)",
    x = "Vaccine coverage (%)",
    y = "Vaccine efficacy (%)",
    fill = NULL
  ) + 
...
ggsave(plot_cases_scenario5_percent_reduction, file = "5_scenario_analysis_v4/scenario_outputs/plots/scen5/plot_cases_scenario5c_percent_reduction.png", width = 7, height = 7)



plot_cases_scenario5_efficiency1 <- ggplot(total_cases_averted_scenario5_join2,
  ...
  geom_text(data = filter(total_cases_averted_scenario5_join2, 
                          vacc_cov_percent %in% c(10, 40, 70, 100),
                          vacc_eff_percent %in% c(10, 40, 70, 100)), 
            aes(label = round(cases_averted_per_1000_vaccine, 3)), size = 6) +
  scale_fill_gradientn(colors = c(
    "#FDE7BD",  # soft yellow
    "#F8B3D0",  # pastel pink
    "#C19BD3",  # muted lavender
...
ggsave(plot_cases_scenario5_efficiency1, file = "5_scenario_analysis_v4/scenario_outputs/plots/scen5/plot_cases_scenario5c_efficiency1.png", width = 7, height = 7)


plot_cases_scenario5_efficiency2 <- ggplot(total_cases_averted_scenario5_join2,
...
ggsave(plot_cases_scenario5_efficiency2, file = "5_scenario_analysis_v4/scenario_outputs/plots/scen5/plot_cases_scenario5c_efficiency2.png", width = 7, height = 7)


# save file for boxplot
boxplot.df.scen5 <- data.frame(
  percent_reduction = total_cases_averted_scenario5_join2$percent_reduction_cases,
  scenario = "Scenario 5 (high transmissibility in hospital)"
)

# save as csv
write.csv(boxplot.df.scen5, "5_scenario_analysis_v4/scenario_outputs/csv/boxplotscen5.csv")

read.csv("5_scenario_analysis_v4/scenario_outputs/csv/boxplotscen5.csv")



# save file for boxplot
boxplot.df.scen5b <- data.frame(
  num_vacc_per_case = total_cases_averted_scenario5_join2$num_vacc_per_one_case_averted,
  scenario = "Scenario 5 (high transmissibility in hospital)"
)

# save as csv
write.csv(boxplot.df.scen5b, "5_scenario_analysis_v4/scenario_outputs/csv/boxplotscen5b.csv")

read.csv("5_scenario_analysis_v4/scenario_outputs/csv/boxplotscen5b.csv")
